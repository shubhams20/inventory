...
if(isset($_POST['payment'])){
					...
					if($validated_data === false) {
							...
							if($count==0)
							{
                                                         $db->query("insert into customer_details(customer_name,customer_address,customer_contact1,customer_contact2) values('$customer','$address','$contact1','$contact2')");   
                                                        }
                                                        $stock_name=$_POST['stock_name'];
							$quty=$_POST['quty'];
							$date=mysql_real_escape_string($_POST['date']);
							$sell=$_POST['sell'];
							$wholesalesell=$POST['wholesalesell'];
							$total=$_POST['total'];
					  ...
					  $autoid="SD".$max."";
						...
						$rate=$_POST['sell'][$i];
						$wholesalesell=$_POST['wholesalesell'][$i];
						$total=$_POST['total'][$i];
			...
			if($count >= 1)
			{  
			$category=  $db->queryUniqueValue("SELECT category FROM stock_details WHERE stock_name='$name1'");
			$db->query("insert into stock_sales (category,tax,tax_dis,discount,dis_amount,grand_total,transactionid,stock_name,selling_price,quantity,amount,date,username,customer_id,subtotal,payment,balance,due,mode,description,count1,billnumber) 
                            values('$category',$tax,'$tax_dis',$discount,$dis_amount,$payable,'$autoid','$name1',$rate,$quantity,$total,'$mysqldate','$username','$customer',$subtotal,$payment,$balance,'$due','$mode','$description',$i+1,'$bill_no')");
			
			$amount = $db->queryUniqueValue("SELECT quantity FROM stock_avail WHERE name='$name1'");
				$amount1 = $amount - $quantity;
			
			$db->query("insert into stock_entries (stock_id,stock_name,quantity,opening_stock,closing_stock,date,username,type,salesid,total,selling_price,wholesale_selling_price,count1,billnumber) values('$autoid','$name1',$quantity,$amount,$amount1,'$mysqldate','$username','sales','$autoid',$subtotal,$rate,$wholesalesell,$i+1,'$bill_no')");
        ...
        $("#item").blur(function()
			{
				if (document.getElementById('retail').checked) {
		        	...
		        	document.getElementById('total').value=document.getElementById('wholesalesell').value * document.getElementById('quty').value
		        }
    });
        $("#item").blur(function()
			{
			 ...
			 $.post('check_item_details.php', {stock_name1: $(this).val() },
				function(data){
								...
								$("#sell").val(data.sell);
								$("#wholesalesell").val(data.wholesalesell);
...
function numbersonly(e){
    ...
    wholesalesell=document.getElementById('wholesalesell').value;
    ...
    $('<tr id='+item+'><td><lable id='+item+'roll class=jibi007 >'+roll+'</label></td><td><input type=hidden value='+item+' id='+item+'id ><input type=text name="stock_name[]"  id='+item+'st style="width: 150px" class="round  my_with" readonly="readonly" ></td><td><input type=text name=quty[] readonly="readonly" value='+quty+' id='+item+'q class="round  my_with" style="text-align:right;" ></td><td><input type=text name=sell[] readonly="readonly" value='+sell+' id='+item+'s class="round  my_with" style="text-align:right;"  ></td><td><input type=text name=wholesalesell[] readonly="readonly" value='+wholesalesell+' id='+item+'w class="round  my_with" style="text-align:right;"  ></td><td><input type=text name=stock[] readonly="readonly" value='+disc+' id='+item+'p class="round  my_with" style="text-align:right;" ></td><td><input type=text name=jibi[] readonly="readonly" value='+total+' id='+item+'to class="round  my_with" style="width: 120px;margin-left:20px;text-align:right;" ><input type=hidden name=total[] id='+item+'my_tot value='+main_total+'> </td><td><input type=button value="" id='+item+' style="width:30px;border:none;height:30px;background:url(images/edit_new.png)" class="round" onclick="edit_stock_details(this.id)"  ></td><td><input type=button value="" id='+item+' style="width:30px;border:none;height:30px;background:url(images/close_new.png)" class="round" onclick=reduce_balance("'+item+'");$(this).closest("tr").remove(); ></td></tr>').fadeIn("slow").appendTo('#item_copy_final');
    document.getElementById('quty').value="";
    document.getElementById('sell').value="";
    document.getElementById('wholesalesell').value="";
    ...
    if(document.getElementById('grand_total').value==""){
        document.getElementById('grand_total').value=parseFloat(parseFloat(main_total).toFixed(2));
    }else{
    document.getElementById('grand_total').value=parseFloat(parseFloat(document.getElementById('grand_total').value).toFixed(2))+parseFloat(main_total);
    }
     document.getElementById('main_grand_total').value=parseFloat(parseFloat(document.getElementById('grand_total').value).toFixed(2));
    document.getElementById('tax').value = parseFloat(parseFloat(document.getElementById('payable_amount').value*0.15).toFixed(2));
    ...
    document.getElementById(id+'w').value=document.getElementById('wholesalesell').value;
    document.getElementById(id+'p').value=document.getElementById('stock').value;
    document.getElementById('grand_total').value=parseFloat(parseFloat(parseFloat(document.getElementById('grand_total').value)+parseFloat(document.getElementById('posnic_total').value)-parseFloat(document.getElementById(id+'my_tot').value)).toFixed(2));
    document.getElementById('main_grand_total').value=parseFloat(parseFloat(document.getElementById('grand_total').value).toFixed(2));
    document.getElementById('tax').value = parseFloat(parseFloat(document.getElementById('payable_amount').value*0.15).toFixed(2));
    ...
    document.getElementById(id+'my_tot').value=parseFloat(parseFloat(document.getElementById('posnic_total').value).toFixed(2));
    document.getElementById('quty').value="";
    document.getElementById('sell').value="";
    document.getElementById('wholesalesell').value="";
    document.getElementById('stock').value="";
   ...
   function edit_stock_details(id) {
    ...
    document.getElementById('wholesalesell').value=document.getElementById(id+'w').value;
...
function balance_amount(){
    ...
    data=parseFloat(parseFloat(document.getElementById('payable_amount').value).toFixed(2));
    document.getElementById('tax').value = parseFloat(parseFloat(document.getElementById('payable_amount').value*0.15).toFixed(2));
    document.getElementById('balance').value=parseFloat(parseFloat(data-parseFloat(document.getElementById('payment').value)).toFixed(2));
...
function calculate_tax(){
    if(document.getElementById('main_grand_total').value!="" && document.getElementById('grand_total').value!=""){
	    document.getElementById('tax').value = parseFloat(parseFloat(document.getElementById('payable_amount').value*0.15).toFixed(2));
...
function reduce_balance(id){
 var minus=(parseFloat(document.getElementById(id+"my_tot").value).toFixed(2));
  document.getElementById('grand_total').value=parseFloat(document.getElementById('grand_total').value)-parseFloat(minus);
...
{ 
 ...
 function print2() 
{ 
 window.open("sales_docket_print.php?start_date=khan","myNewWinsr","width=620,height=800,toolbar=0,menubar=no,status=no,resizable=yes,location=no,directories=no,scrollbars=yes"); 
}
function quote1()
{ 

 var name = <?php echo json_encode($line4->name); ?>;
 var address = <?php echo json_encode($line4->address); ?>;
 var city = <?php echo json_encode($line4->city); ?>;
 var place = <?php echo json_encode($line4->place); ?>;
 var email = <?php echo json_encode($line4->email); ?>;
 var phone = <?php echo json_encode($line4->phone); ?>;
 var mobile = <?php echo json_encode($line4->mobile); ?>;
 var gst = <?php echo json_encode($line4->gst); ?>;
 var arr1 = []

var table = document.getElementById("item_copy_final");
                ...
                alert(document.getElementById(arr1[0]+"st").value);
 var arrLength = arr1.length;

var myTable = "<form><input type='button' value='Print' onClick='window.print()'></form><br /><br />"; 

myTable+= "<table width='100%' border='0' cellspacing='0' cellpadding='0'>";
    myTable+= "<tr><td align='center'>";
    myTable+=" \
<div align='left'><strong>"+name+"</strong><br />\
                  ...
                  GST<strong>:"+gst+"</strong><br/ ><br/ ><br/ ><br/ ><br/ >\
        <tr>\
   ";
   myTable+="\
  ...
  for (var i=0; i<arrLength; i++) {
    myTable+="<tr><td>"+document.getElementById(arr1[i]+"st").value+"</td>";
    myTable+="<td>"+document.getElementById(arr1[i]+"q").value+"</td>";
    myTable+="<td>"+document.getElementById(arr1[i]+"to").value+"</td></tr>";
  } 
  myTable+="</table></td></tr><tr><td>&nbsp;</td></tr><tr><td>&nbsp;</td></tr></table></td></tr></table>";         
  myTable+="\
<table>\
<tr>\
              <td width='150'><strong>Grand Total</strong></td>\
              <td width='150'>&nbsp;"+document.getElementById("grand_total").value+"</td>\
            </tr>\
            <tr>\
              <td width='150'><strong>Discount(%)</strong></td>\
              <td width='150'>&nbsp;"+document.getElementById("discount").value+"</td>\
            </tr>\
            <tr>\
              <td width='150'><strong>Payable Amount (After Tax)</strong></td>\
              <td width='150'>&nbsp;"+document.getElementById("payable_amount").value+"</td>\
            </tr>\
          </table></td>\
        </tr>\
        <tr>\
				...
				<form name="form1" method="post" id="form1" action="">
                  ...
                  <table class="form "  border="0" cellspacing="0" cellpadding="0">
                    <tr>  <td>&nbsp; </td> <td>&nbsp; </td>
                      ...
                      <td>Date:</td>
                      <td><input  name="date"  placeholder="" value= "<?php date_default_timezone_set('NZ'); echo date('d-m-Y');?>" type="text" id="name" maxlength="200"  class="round default-width-input"  /></td>
                      <td>&nbsp; </td>  <td>&nbsp; </td>
                    ...
                    <tr>  <td>&nbsp; </td> <td>&nbsp; </td>
                      ...
                      <td><input name="address" placeholder="ENTER ADDRESS" type="text" id="address" maxlength="200"  class="round default-width-input"  /></td>
                       ...
                       <td>&nbsp; </td> <td>&nbsp; </td> <td >Contact1:&nbsp; &nbsp; &nbsp; </td>
                      <td><input name="contact1" placeholder="ENTER CONTACT" type="text" id="contact1" maxlength="25"  class="round default-width-input" style="width:120px " /></td>
                    ...
                    <tr>
                     <td>&nbsp; </td> <td>&nbsp; </td>
                      ...
                      <td>Email:</td>
                      <td><input name="contact2" placeholder="ENTER CONTACT" type="text" id="contact2" maxlength="25"  class="round default-width-input"  style="width:120px " /></td>
                  ...
                  <table class="form" id="itemtable">
                        ...
                        <td><input name="quty"  type="text" id="quty"  maxlength="200"   class="round default-width-input my_with" onKeyPress="quantity_chnage(event);return numbersonly(event)" onkeyup="total_amount();unique_check();stock_size();"    /></td>
                      	<td>
                          	<input type="radio" name="customertype" id="retail" value="re" onClick="total_amount();" checked> Retail<br>
  							<input type="radio" name="customertype" id"wholesale" value="wh"  onClick="total_amount();">Wholesale<br>
                          </td>
                      ...
                      <td><input name="price"  type="text" id="wholesalesell" readonly maxlength="200"  class="round default-width-input my_with"   /></td>
                      ...
                      <td><input type="button" onclick="add_values()" onkeyup=" balance_amount();calculate_tax();" id="add_new_code"  style="margin-left:30px; width:30px;height:30px;border:none;background:url(images/add_new.png)" class="round"> </td>
                  ...
                  <tr> <td>Mode &nbsp;</td><td>
                      ...
                      <td>
                       Due Date:<input type="text" name="duedate" id="test2" value="<?php date_default_timezone_set('NZ'); echo date('d-m-Y');?>" class="round">
                  </td>
                  <td> Tax:<input type="text" name="tax" readonly id="tax"></td> 
                 <td>Tax Description:<input type="text" name="tax_dis" value="15% tax is included" readonly="true"> </td>
                  ...
                  <table class="form">
                    <tr>
                     ...
                     <input class="button round red   text-upper"  type="reset" name="Reset" value="Reset"> </td>
                      ...
                      <td><input class="button round red   text-upper"  type="button" name="deliverydocket" value="Delivery Docket" onClick='print2();'> </td>
                      <td><input class="button round red   text-upper"  type="button" name="quotation" value="Quotation" onClick='quote1();'> </td>
