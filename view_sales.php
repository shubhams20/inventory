...
{
$SQL = "SELECT DISTINCT(transactionid) FROM  stock_sales WHERE transactionid LIKE '%".$_POST['searchtxt']."%' ";
...
$query = "SELECT COUNT(transactionid) as num FROM stock_sales WHERE transactionid LIKE '%".$_POST['searchtxt']."%'";
...
if(isset($_GET['limit']) && is_numeric($_GET['limit'])){
	...
	$sql =  "SELECT * FROM stock_sales ORDER BY LPAD(REPLACE(`stock_sales`.`transactionid`,'SD ',''),10,'0') DESC  ";
	if(isset($_POST['Search']) AND trim($_POST['searchtxt'])!="")
{

	$sql= "SELECT * FROM stock_sales WHERE transactionid LIKE '%".$_POST['searchtxt']."%'  ORDER BY date desc LIMIT $start, $limit";
...
while($row = mysql_fetch_array($result)) 
{
 ?> 
   ...
   <td> <?php echo $row['subtotal']; ?> </td>
	...
	<td>
        <a href="update_sales.php?sid=<?php echo $row['id'];?>"	class="table-actions-button ic-table-edit">
	</a>	
	</td>
...
<td><a href='sales_invoice_delivery_docket.php?sellid="<?php echo $row['transactionid'];?>"' target="_blank">Delivery Docket</a>
    </td>
