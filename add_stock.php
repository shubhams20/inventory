...
<head>
	...
	<!-- jQuery & JS files -->
	 <?php
   		$db->query("SELECT supplier_name FROM supplier_details");
	 	while ($line = $db->fetchNextObject()) {
	  
		  		$val = $line->supplier_name;
	...
	$(document).ready(function() {
		...
		$("#form1").validate({
			...
			messages: {
				...
				wholesalesell: {
					required: "Please Enter WholeSale Selling Price",
					minlength: "Category Name must consist of at least 3 characters"
				},
				cost: {
				...
				sell: {
					required: "Please Enter Retail Selling Price",
					minlength: "Category Name must consist of at least 3 characters"
				},
				category: {
			...
			<div class="side-content fr">
				...
				<div class="content-module">
					...
					$gump->validation_rules(array(
						...
						'wholesalesell'    => 'required|max_len,200',
						'cost'     => 'required|max_len,200',
						'supplier'     => 'required|max_len,200',
						'category'     => 'required|max_len,200'
					...
					$gump->filter_rules(array(
						...
						'wholesalesell'     => 'trim|sanitize_string|mysql_escape',
					...
					if($validated_data === false) {
							...
							$wholesalesell=mysql_real_escape_string($_POST['wholesalesell']);
...
<link rel="stylesheet"  href="dist/js/jquery.alerts.css" >
			...
			{
			$r = $db->queryUniqueValue("select EXISTS(SELECT * FROM stock_avail WHERE name='$name')");
			$s = $db->queryUniqueValue("select EXISTS(SELECT * FROM stock_details WHERE stock_name='$name')");

			if($r>0 || $s > 0) 
				echo "<br><font color=red size=+1 >Item with this name exists, choose different name !</font>" ;
			

			else {
			if($db->query("insert into stock_details(stock_id,stock_name,stock_quatity,supplier_id,company_price,selling_price,wholesale_selling_price,category) values('$stockid','$name',0,'$supplier',$cost,$sell,$wholesalesell,'$category')"))
			{ 
				...
				header("Location: add_stock.php?msg=$msg");
                       ...
                       $db->query("delete from stock_avail where name='$name'");
                       $db->query("delete from stock_details where stock_name='$name'");
				...
				<form name="form1" method="post" id="form1" action="">
                  ...
                  <table class="form"  border="0" cellspacing="0" cellpadding="0">
                    ...
                    <tr>
                      ...
                      <td><input name="sell" placeholder="ENTER RETAIL SELLING PRICE" type="text" id="sell" maxlength="200"  class="round default-width-input" onkeypress="return numbersonly(event)" value="<?php echo $sell; ?>" /></td>
                       
                      <td><span class="man">*</span>WholeSale Sell:</td>
                      <td><input name="wholesalesell" placeholder="ENTER WHOLESALE SELLING PRICE" type="text" id="wholesalesell" maxlength="200"  class="round default-width-input" onkeypress="return numbersonly(event)" value="<?php echo $wholesalesell; ?>" /></td>
                       
                    </tr>
                    <tr>
                      <td><span class="man">*</span>Supplier:</td>
                      <td><input name="supplier" placeholder="ENTER SUPPLIER" type="text" id="supplier"  maxlength="200"  class="round default-width-input"  style="width:130px" list="languageList" value="<?php echo $supplier; ?>" /></td>
                     <datalist id="languageList">
                      ...
                      <?php
						 	...
						 	while ($line = $db->fetchNextObject()) {
							  		...
							  		?><option value="<?php echo $line->supplier_name;;?>" />
								<?php
							 	}
					    ?>
                      ...
                      <td><input name="category" placeholder="ENTER CATEGORY NAME" type="text" id="category" maxlength="200"  class="round default-width-input" list="languageList2" value="<?php echo $category; ?>" /></td>
                       <datalist id="languageList2">
                    
                      <?php
					   		$db->query("SELECT category_name FROM category_details");
						 	while ($line = $db->fetchNextObject()) {
						  
							  		?><option value="<?php echo $line->category_name;;?>" />
								<?php
							 	}
					    ?>
