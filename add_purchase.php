...
include_once("init.php");
	if(isset($_POST['supplier']) and isset($_POST['stock_name'])){
					...
					if($validated_data === false) {
							...
							$sell=$_POST['sell'];
							$wholesalesell=$_POST['wholesalesell'];
							$cost=$_POST['cost'];
							$total=$_POST['total'];
			...
			$db->query("insert into stock_details(stock_id,stock_name,stock_quatity,supplier_id,company_price,selling_price,wholesale_selling_price) values('$autoid','$stock_name[$i]',0,'$supplier',$cost[$i],$sell[$i],$wholesalesell[$i])");


			$db->query("INSERT INTO stock_entries(stock_id,stock_name,stock_supplier_name,quantity,company_price,selling_price,wholesale_selling_price,opening_stock, closing_stock, date, username, type,salesid, total, payment, balance, mode, description, due, subtotal,count1,billnumber) VALUES ('$autoid1','$stock_name[$i]','$supplier',$quty[$i],$cost[$i],$sell[$i],$wholesalesell[$i],0,$quty[$i],'$date','$username','entry','pr12' ,$total[$i],$payment,$balance,'$mode','$description','$due',$subtotal,$i+1,'$bill_no')"); 
			...
			$db->query("INSERT INTO stock_entries(stock_id,stock_name,stock_supplier_name,quantity,company_price,selling_price,wholesale_selling_price,opening_stock,closing_stock,date,username,type,salesid,total,payment,balance,mode,description,due,subtotal,count1,billnumber) VALUES ('$autoid1','$stock_name[$i]','$supplier',$quty[$i],$cost[$i],$sell[$i], $wholesalesell[$i],$amount,$amount1,'$date','$username','entry','pr12',$total[$i],$payment,$balance,'$mode','$description','$due',$subtotal,$i+1,'$bill_no')"); 
			...
			//VALUES (NULL, '$autoid1', '$stock_name[$i]', '$supplier', '', '$quantity', '$brate', '$srate', '$amount', '$amount1', '$mysqldate', 'sdd', 'entry', 'Sa45', '432.90', '2342.90', '24.34', 'cash', 'sdflj', '2010-03-25 12:32:02', '45645', '1');
                        ...
                        $msg="Purchase order Added successfully Ref: ". $_POST['stockid']."" ;
...
{ 
 ...
 +"&item="+$('#stock_name[]').val()	 +"&quty="+$('#quty[]').val()	+"&cost="+$('#cost[]').val() +"&total="+$('#jibi[]').val()		+"&payment="+$('#payment').val()
...
function numbersonly(e){
    ...
    $('<tr id='+item+'><td><lable id='+item+'roll class=jibi007 >'+roll+'</label></td><td><input type=hidden readonly=readonly value='+item+' id='+item+'id ><input type=text name="stock_name[]"  id='+item+'st style="width: 150px" class="round  my_with" ></td><td><input type=text name=quty[] readonly="readonly" value='+quty+' id='+item+'q class="round  my_with" style="text-align:right;" ></td><td><input type=text name=cost[] readonly="readonly" value='+cost+' id='+item+'c class="round  my_with" style="text-align:right;"></td><td><input type=text name=sell[] readonly="readonly" value='+sell+' id='+item+'s class="round  my_with" style="text-align:right;"  ></td><td><input type=text name=wholesalesell[] readonly="readonly" value='+wholesalesell+' id='+item+'w class="round  my_with" style="text-align:right;"  ></td><td><input type=text name=stock[] readonly="readonly" value='+disc+' id='+item+'p class="round  my_with" style="text-align:right;" ></td><td><input type=text name=jibi[] readonly="readonly" value='+total+' id='+item+'to class="round  my_with" style="width: 120px;margin-left:20px;text-align:right;" ><input type=hidden name=total[] id='+item+'my_tot value='+main_total+'> </td><td><input type=button value="" id='+item+' style="width:30px;border:none;height:30px;background:url(images/edit_new.png)" class="round" onclick="edit_stock_details(this.id)"  ></td><td><input type=button value="" id='+item+' style="width:30px;border:none;height:30px;background:url(images/close_new.png)" class="round" onclick=reduce_balance("'+item+'");$(this).closest("tr").remove(); ></td></tr>').fadeIn("slow").appendTo('#item_copy_final');
    ...
    document.getElementById('grand_total').value=parseFloat((parseFloat(document.getElementById('grand_total').value)+parseFloat(main_total)).toFixed(2));
    }
    ...
    document.getElementById('grand_total').value=parseFloat(parseFloat(parseFloat(document.getElementById('grand_total').value)+
    	parseFloat(document.getElementById('posnic_total').value)-
    	parseFloat(document.getElementById(id+'my_tot').value)).toFixed(2));
    document.getElementById('main_grand_total').value=parseFloat(parseFloat(document.getElementById('grand_total').value).toFixed(2));
 ...
 var minus=(parseFloat(document.getElementById(id+"my_tot").value).toFixed(2));
  document.getElementById('grand_total').value=parseFloat(parseFloat(document.getElementById('grand_total').value).toFixed(2))-minus;
  document.getElementById('main_grand_total').value=parseFloat(parseFloat(document.getElementById('grand_total').value).toFixed(2));
...
function balance_amount(){
    ...
    data=parseFloat(parseFloat(document.getElementById('grand_total').value).toFixed(2));
    document.getElementById('balance').value=parseFloat(parseFloat(data-parseFloat(document.getElementById('payment').value)).toFixed(2));
        ...
        if(document.getElementById('grand_total').value!=""){
         ...
         document.getElementById('payment').value=parseFloat(parseFloat(document.getElementById('grand_total').value).toFixed(2));
        }else{
            document.getElementById('balance').value='000.00';
...
<link rel="stylesheet"  href="dist/js/jquery.alerts.css" >
            ...
            <div class="mytable_row ">
                 ...
                 <td>&nbsp;</td>
                      ...
                      <td><input name=""  type="text" id="wholesalesell" readonly maxlength="200"  class="round default-width-input my_with"   /></td>
